  ...
  return (
    <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="flex space-x-2">
              {(['all', 'today', 'month'] as const).map(f => (
                <button key={f} onClick={() => setDateFilter(f)} className={`px-3 py-1 rounded-full text-sm font-medium ${dateFilter === f ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
          ...
          <div>
            <div className="flex items-center mb-2">
              ...
              <h3 className="font-medium">Filter by Payment Method</h3>
            </div>
            <select
              value={paymentMethodFilter}
              onChange={(e) => setPaymentMethodFilter(e.target.value as any)}
              className="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-transparent w-full md:w-auto"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 flex items-center">
          <div className="p-3 bg-blue-100 rounded-full mr-4"><ShoppingCart className="w-6 h-6 text-blue-600" /></div>
          <div>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div className="overflow-x-auto">
          ...
          {!loading && filteredPurchases.length === 0 && (
            <div className="text-center py-8 text-gray-500">No sales data found for the selected filters.</div>
          )}
        </div>
      </div>
    </div>
